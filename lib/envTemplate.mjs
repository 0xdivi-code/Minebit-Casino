...
#  ------------------------------
#  Every credential below is intentionally empty. This deployment runs on a
#  simulated front-end only, so games, the cashier, the sportsbook, loyalty and
#  account services stay locked until the real keys are supplied.
#
#  To request full access (live keys, provider accounts, deployment support):
#      Telegram: {{CONTACT_HANDLE}}  ->  {{CONTACT_URL}}
#
#  Usage:
...
/** Env var names, so UI copy can reference them without magic strings. */
export const CREDENTIAL_KEYS = {
  gameAggregator: "GAME_AGGREGATOR_API_KEY",
  sportsbook: "SPORTSBOOK_API_KEY",
  custody: "CUSTODY_API_KEY",
  auth: "NEXTAUTH_SECRET",
  telegram: "TELEGRAM_BOT_TOKEN",
  kyc: "KYC_PROVIDER_API_KEY",
  loyalty: "LOYALTY_API_KEY",
  provablyFair: "PROVABLY_FAIR_SERVER_SEED",
  database: "DATABASE_URL",
};

/** Credentials that must be connected before the platform actually works. */
export const REQUIRED_CREDENTIALS = [
  { key: CREDENTIAL_KEYS.gameAggregator, label: "Game launches", area: "Casino & Originals" },
  { key: CREDENTIAL_KEYS.sportsbook, label: "Odds & live betting", area: "Sportsbook" },
  { key: CREDENTIAL_KEYS.custody, label: "Deposits & withdrawals", area: "Cashier" },
  { key: CREDENTIAL_KEYS.auth, label: "Sessions & accounts", area: "Auth" },
  { key: CREDENTIAL_KEYS.telegram, label: "Telegram login & bot", area: "Auth" },
  { key: CREDENTIAL_KEYS.kyc, label: "Verification, KYC & AML", area: "Compliance" },
  { key: CREDENTIAL_KEYS.loyalty, label: "Loyalty, promos & Profit Share", area: "Rewards" },
  { key: CREDENTIAL_KEYS.provablyFair, label: "Provably fair RNG", area: "Originals" },
  { key: CREDENTIAL_KEYS.database, label: "Accounts, bets & history", area: "Infrastructure" },
];
